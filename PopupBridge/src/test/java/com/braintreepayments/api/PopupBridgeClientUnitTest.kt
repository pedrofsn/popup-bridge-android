...
class PopupBridgeClientUnitTest {
    ...
    @Test
    fun `when onOpen is called with a Venmo URL, appSwitchHandler launchApp is called instead of openUrl`() {
        initializeClient()
    ...
    @Test
    fun `when no app switch deep link on return, canceled JS is run`() = runTest {
        every { intent.data } returns null
        initializeClient {
    ...
    @Test
    fun `when handleReturnToApp completes via browser switch, clearReturnIntentIfPresent is not called`() =
        runTest {
