...
internal class AppSwitchHandler(
    ...
    /**
     * Returns true if an app-switch return was expected and has now been consumed (canceled).
     * Returns false (no-op) if no app-switch was in flight, so the caller can fall through to
     * its own return-handling logic (e.g. a browser-switch completion).
     */
    fun handleNoResult(): Boolean {
        if (!expectingAppSwitchReturn) return false
